...
//            subgraph.addEdge(v, v); // add self-loop for the selected vertex
    ...
    public int computeDiameterOfKTruss(int k) {
        int maxDiameter = 0;

        for (int v : g.keySet()) {
            for (int u : g.get(v).keySet()) {
                int diameter = findDiameterOfKTruss(v, u, k);
    ...
    private int findDiameterOfKTruss(int start, int end, int k) {
        // Implement the logic to find the diameter of k-truss between start and end vertices
        // You may use BFS or any other appropriate algorithm
        // Return the diameter of the k-truss community
        return 0;  // Placeholder, replace with actual implementation
    }

    public int computeDiameterOfKTTC(int k) {
        int maxDiameter = 0;

        for (int v : g.keySet()) {
            for (int u : g.get(v).keySet()) {
                int diameter = findDiameterOfKTTC(v, u, k);
                if (diameter > maxDiameter) {
                    maxDiameter = diameter;
                }
            }
    ...
    private int findDiameterOfKTTC(int start, int end, int k) {
        // Implement the logic to find the diameter of k-TTC between start and end vertices
        // You may use BFS or any other appropriate algorithm
        // Return the diameter of the k-TTC community
        return 0;  // Placeholder, replace with actual implementation
    }

// Add these methods to the Main class

    public static void runDiameterExperiment(MyGraph g, int maxK) {
        System.out.println("Diameter Experiment Results:");
        System.out.println("K\tK-Truss Diameter\tK-TTC Diameter");

        for (int k = 2; k <= maxK; k++) {
            int kTrussDiameter = g.computeDiameterOfKTruss(k);
            int kTTCDiameter = g.computeDiameterOfKTTC(k);

            System.out.println(k + "\t" + kTrussDiameter + "\t" + kTTCDiameter);
        }
    }

    public double calculateMeanDiameter(List<MyGraph> communities) {
        double sumDiameter = 0;
        for (MyGraph community : communities) {
            int diameter = calculateDiameter(community);
            sumDiameter += diameter;
        }
        return sumDiameter / communities.size();
    }

    public int calculateDiameter(MyGraph community) {
        int maxDiameter = 0;
        Set<Integer> vertices = community.getVertexSet();
        for (Integer source : vertices) {
            for (Integer destination : vertices) {
                if (!source.equals(destination)) {
                    int diameter = bfsDiameter(source, destination, community);
                    if (diameter > maxDiameter) {
                        maxDiameter = diameter;
                    }
                }
    ...
    private int bfsDiameter(Integer source, Integer destination, MyGraph community) {
        ...
        while (!queue.isEmpty()) {
            ...
            for (Integer neighbor : community.getAddjList(current)) {
        ...
        // If the destination is not reachable, return -1 indicating infinite diameter
